...
class Adapters(containers.DeclarativeContainer):
    ...
    read_inmemory_transform = providers.Singleton(TeamStatsMockReadTransform)
    write_database_transform = providers.Singleton(TeamStatsBigqueryWriteTransform, pipeline_options=pipeline_options)

    team_stats_inmemory_io_connector = providers.Singleton(
        TeamStatsMockIOAdapter,
        read_transform=read_inmemory_transform)

    team_stats_database_io_connector = providers.Singleton(
        TeamStatsBigqueryIOAdapter,
        write_transform=write_database_transform)
